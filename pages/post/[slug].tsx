        ...
        <main>
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <p className="font-extralight text-sm">
                        Blog post by <span className='text-green-600'>
                            {post.author.name}
                        </span> - Publised at {new Date(post._createdAt).toLocaleString()}
                    </p>
                </div>
                <div className="mt-10">
                    <PortableText
            ...
            <form className=' flex flex-col p-5 max-w-2xl mx-auto mb-10' onSubmit={handleSubmit(onSubmit)}>
                ...
                <label className='block mb-5'>
                    ...
                    <input {...register("email", { required: true })} className="shadow border rounded py-2 px-3 form-input mt-1 block w-full ring-yellow-500 outline-none focus:ring " placeholder='Your Email' type='email' />
                </label>
                <label className='block mb-5'>
                ...
                <input type="submit" className='shadow bg-yellow-500 hover:bg-yellow-400 focus:shadow-outline focus:outline-none text-white hover:text-black font-bold py-2 px-4 rounded cursor-pointer' />
            </form>
        </main>
    )
}
...
export const getStaticPaths = async () => {
    ...
    const postpage = await sanityClient.fetch(query)
    const paths = postpage.map((p: Post) => ({
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  image
    ...
    return {
        ...
        revalidate: 60 // after 60s it will update the cache
    };
};
